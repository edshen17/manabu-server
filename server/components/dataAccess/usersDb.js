function makeUsersDb({ makeDb, User, Teacher, Package }) {
  ...
  async function joinUserTeacher(user) {
  ...
  async function findById(id, currentAPIUser) {
    ...
    if (user) return joinUserTeacher(user);
    else throw new Error('No user found.');
  }
  async function findOne(attrObj, currentAPIUser) {
    const db = await makeDb();
