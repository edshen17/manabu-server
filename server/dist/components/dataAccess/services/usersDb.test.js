...
exports.createFakeDbUser = void 0;
...
const _createNewDbDoc = async (asyncCallback) => {
    const newDoc = await asyncCallback;
    return newDoc;
};
const createNewDbUser = async (email) => {
    const fakeUserEntity = user_1.makeUserEntity.build({
...
const createNewDbTeacher = async (dbUser) => {
    const fakeTeacherEntity = user_1.makeTeacherEntity.build({ userId: dbUser._id });
...
const createNewDbPackage = async (dbUser) => {
    const fakePackageEntity = package_1.makePackageEntity.build({
        hostedBy: dbUser._id,
        lessonAmount: 5,
        isOffering: true,
        packageType: 'light',
...
const createFakeDbUser = async (isTeacher) => {
...
exports.createFakeDbUser = createFakeDbUser;
before(async () => {
    userDbService = await index_1.makeUserDbService;
    packageDbService = await index_1.makePackageDbService;
    teacherDbService = await index_1.makeTeacherDbService;
});
describe('userDb service', () => {
    describe('findById', () => {
        ...
        it('given an existing user id (viewing teacher as admin), should have additional restricted data', async () => {
            const accessOptionsCopy = JSON.parse(JSON.stringify(accessOptions));
