...
function endAppointments() {
    Appointment.find({ to: { $lte:  new Date().toISOString() }, isPast: false,})
    .lean()
    .exec((appointments) => {
        if (appointments) {
                            ...
                            .exec((searchBank) => {
                                if (searchBank) {
                                    ...
                                    if (appointments[i].status == 'confirmed') { // add to minute bank only if confirmed
                                        ...
                                        MinuteBank.findOneAndUpdate(participants, updateMinuteBank).lean().catch((err) => {console.log(err)})
                                    ...
                                    PackageTransaction.findOneAndUpdate({ _id: packageTransaction._id }, 
                                        updatePackageTransaction).lean().catch((err) => {console.log(err)})
                                }
                            }).catch((err) => {console.log(err)})   
                        }
                    }
                })
                Appointment.findByIdAndUpdate(appointments[i]._id, { isPast: true }).lean().catch((err) => {console.log(err)})
            }
        }
    })
}
